...
import { fetchResumes, fetchTree, readFile, statFile, FileNode } from "@/lib/api";
...
const STORAGE_KEY = "resume-platform-current-resume";

export default function Editor() {
  const [resumeList, setResumeList] = useState<string[]>([]);
  const [currentResume, setCurrentResume] = useState<string>("default");

  // Restore last-used resume from localStorage after hydration
  ...
  // Flag: should we trigger an initial compile after loading a new resume?
  const needsInitialCompile = useRef(false);

  const { status, result, pdfSrc, scheduleCompile, compileNow, compileOnly } =
    useCompiler(currentResume, selectedFile);
  ...
  // Fetch tree when resume changes
  useEffect(() => {
    ...
    setLoadError("");
    mtimeRef.current = 0;
    needsInitialCompile.current = true;
    localStorage.setItem(STORAGE_KEY, currentResume);
    fetchTree(currentResume)
      .then(setTree)
      .catch(() => setLoadError("Could not load file tree."));
  }, [currentResume]);

  // Load file content + mtime when selection or resume changes
  useEffect(() => {
    if (!selectedFile || !currentResume) return;
    setContent("");
    readFile(currentResume, selectedFile)
      .then((text) => {
        setContent(text);
        // Trigger initial compile on resume switch so PDF preview is populated
        if (needsInitialCompile.current) {
          needsInitialCompile.current = false;
          compileNow(text);
        }
        return statFile(currentResume, selectedFile);
      })
      .then((mtime) => { mtimeRef.current = mtime; })
      .catch(() => setContent("// Could not load file"));
  }, [selectedFile, currentResume]); // eslint-disable-line react-hooks/exhaustive-deps

  // After each compile cycle, update our known mtime so our own saves
  // don't trigger a false external-change reload
  useEffect(() => {
    if ((status === "success" || status === "error") && selectedFile && currentResume) {
      statFile(currentResume, selectedFile)
        .then((mtime) => { mtimeRef.current = mtime; })
        .catch(() => {});
    }
  }, [status, selectedFile, currentResume]);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      ...
      try {
        const mtime = await statFile(currentResume, selectedFile);
        if (mtime > mtimeRef.current + 0.001) {
          const text = await readFile(currentResume, selectedFile);
          setContent(text);
  ...
  }, [selectedFile, currentResume, compileOnly]);
  ...
  const handleSwitchResume = useCallback((name: string) => {
